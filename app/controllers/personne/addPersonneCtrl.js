myApp.controller('addPersonneCtrl', function($scope, LxNotificationService, personneService, $location) {
   ...
   $scope.addPersonne = function(){
        if($scope.addPersonneForm.$valid === true){
            ...
            personneService.addPersonne(personneData).then(function(result){
                 personneService.getPersonnes().then(function(result){
                    $scope.listePersonnes = result.data;
                    $location.path('/personne');
                    LxNotificationService.notify('Personne ajoutée', undefined, undefined, undefined, undefined, undefined, 2 * 2000);
                 });
             })
        }
        else{
